namespace StreetNameRegistry.Projector.Snapshots
{
    ...
    public class SnapshotsController : ApiController
    {
        ...
        private const string SnapshotVerificationsTableName = "[StreetNameRegistry].[SnapshotVerificationStates]";
        private const string SnaphotsTableName = "[StreetNameRegistry].[Snapshots]";
            ...
            var differenceInDaysOfLastVerification =
                await sqlConnection.QuerySingleAsync<int>(
                    $"SELECT DATEDIFF(DAY," +
                        ...
                        $"(SELECT MAX(Created) FROM [StreetNameRegistry].[Snapshots])) As DaysDiff",
                    cancellationToken);
