namespace StreetNameRegistry.Tests.BackOffice.Api.WhenChangingStreetNameName
{
    using System;
    ...
    public sealed class GivenMunicipalityExists : BackOfficeApiTest<StreetNameController>
    {
        ...
        public async Task ThenAcceptedWithLocationIsReturned()
        {
            ...
            MockMediatorResponse<ChangeStreetNameNamesSqsRequest, LocationResult>(expectedLocationResult);

            var request = new ChangeStreetNameNamesRequest
            {
                Straatnamen = new Dictionary<Taal, string> { { Taal.NL, "Bosstraat" } }
            };

            var persistentLocalId = 123;
            var result = (AcceptedResult) await Controller.ChangeStreetNameNames(
                MockValidIfMatchValidator(),
                MockPassingRequestValidator<ChangeStreetNameNamesRequest>(),
                persistentLocalId,
                request,
            ...
            MockMediator.Verify(x =>
                x.Send(
                    It.Is<ChangeStreetNameNamesSqsRequest>(sqsRequest =>
        ...
        {
            MockMediator
                .Setup(x => x.Send(It.IsAny<ChangeStreetNameNamesSqsRequest>(), CancellationToken.None))
                .Throws(new AggregateIdIsNotFoundException());

            var persistentLocalId = 123;
            Func<Task> act = async () =>
            {
                await Controller.ChangeStreetNameNames(
                    MockValidIfMatchValidator(),
                    MockPassingRequestValidator<ChangeStreetNameNamesRequest>(),
                    persistentLocalId,
                    new ChangeStreetNameNamesRequest(),
                    string.Empty,
                    CancellationToken.None);
            };
        ...
        {
            var result = await Controller.ChangeStreetNameNames(
