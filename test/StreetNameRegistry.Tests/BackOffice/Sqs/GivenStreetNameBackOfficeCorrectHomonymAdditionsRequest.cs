namespace StreetNameRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenStreetNameBackOfficeCorrectHomonymAdditionsRequest : StreetNameRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenStreetNameBackOfficeCorrectHomonymAdditionsRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sqsRequest = new CorrectStreetNameHomonymAdditionsSqsRequest()
            {
                PersistentLocalId = Fixture.Create<PersistentLocalId>(),

                Request = new CorrectStreetNameHomonymAdditionsRequest()
                {
        ...
        public void WithNoMunicipalityFoundByStreetNamePersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                ...
                {
                    Request = Fixture.Create<CorrectStreetNameHomonymAdditionsRequest>()
                }, CancellationToken.None);
