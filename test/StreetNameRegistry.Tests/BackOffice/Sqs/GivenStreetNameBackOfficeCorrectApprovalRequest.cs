namespace StreetNameRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenStreetNameBackOfficeCorrectApprovalRequest : StreetNameRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenStreetNameBackOfficeCorrectApprovalRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sut = new CorrectStreetNameApprovalSqsHandler(
                sqsQueue.Object,
                ticketingMock.Object,
                ticketingUrl,
                _backOfficeContext);

            var sqsRequest = new CorrectStreetNameApprovalSqsRequest
            {
                Request = new CorrectStreetNameApprovalRequest
                {
        ...
        public void WithNoMunicipalityFoundByStreetNamePersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                new CorrectStreetNameApprovalSqsRequest
                {
                    Request = Fixture.Create<CorrectStreetNameApprovalRequest>()
                }, CancellationToken.None);
