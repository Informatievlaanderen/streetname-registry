namespace StreetNameRegistry.Tests.BackOffice.Sqs
{
    ...
    public sealed class GivenStreetNameBackOfficeRenameRequest : StreetNameRegistryTest
    {
        private readonly TestBackOfficeContext _backOfficeContext;

        public GivenStreetNameBackOfficeRenameRequest(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
        {
        ...
        public async Task ThenTicketWithLocationIsCreated()
        {
            ...
            var sqsRequest = new RenameStreetNameSqsRequest
            {
                PersistentLocalId = Fixture.Create<PersistentLocalId>(),
                Request = new RenameStreetNameRequest
                {
                    DoelStraatnaamId = $"https://data.vlaanderen.be/id/straatnaam/{Fixture.Create<PersistentLocalId>() + 1}"
                }
            };
        ...
        public void WithNoMunicipalityFoundByStreetNamePersistentLocalId_ThrowsAggregateIdNotFound()
        {
            ...
            var act = async () => await sut.Handle(
                ...
                {
                    Request = Fixture.Create<RenameStreetNameRequest>()
                }, CancellationToken.None);
